...
const StreamForm = ({ stream, onSave, onCancel }) => {
    ...
    const handleChange = (e) => {
        ...
        if (name === 'level' && !stream) {
            const defaultDurations = { Bachelors: '4', Masters: '2', Diploma: '3' };
            setFormData(prev => ({ ...prev, level: value, durationYears: defaultDurations[value] || prev.durationYears }));
            return;
        }
        ...
        <Modal onClose={onCancel}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                    ...
                    <input name="name" value={formData.name} onChange={handleChange} placeholder="e.g., Computer Science" className="w-full p-3 bg-gray-50 dark:bg-gray-800 border-0 rounded-2xl focus:ring-2 focus:ring-primary-500" required />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-black uppercase tracking-widest text-gray-500 mb-1.5 ml-1">Program Type</label>
                    ...
                    <div>
                        <label className="flex items-center gap-1 text-xs font-black uppercase tracking-widest text-gray-500 mb-1.5 ml-1">
...
export default function StreamManager() {
  ...
  const toggleSemester = (streamId, semesterId) => {
    ...
    const levelOrder = ['Bachelors', 'Masters', 'Diploma'];
    const groupedStreams = levelOrder.map(level => ({
        level,
        label: levelLabels[level] || level,
        items: streams.filter(s => s.level === level)
    }));
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm font-bold uppercase tracking-widest">Academic Streams & Subject Plans</p>
                ...
                {isAdmin && (
                        <div className="flex gap-2">
                                <button onClick={() => handleExportWithNotification(addToast, '/api/streams/export', 'streams-data', {}, 'csv')} className="px-4 py-3 bg-gray-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest">Export</button>
        ...
        <div className="space-y-10">
            ...
            <div key={group.level} className="space-y-4">
                <div className="flex items-center gap-3">
                    <span className="text-lg md:text-xl font-black uppercase tracking-[0.25em] text-gray-700 dark:text-gray-200 relative">
                        {group.label}
                        <span className="absolute -bottom-1 left-0 w-10 h-1 bg-primary-500/60 rounded-full animate-pulse"></span>
                    </span>
                    <span className="text-[0.65rem] font-black uppercase tracking-widest bg-gray-100 dark:bg-gray-700 text-gray-500 px-2 py-0.5 rounded-md">{group.items.length} Streams</span>
                </div>
                <div className="grid grid-cols-1 gap-6">
                {group.items.map((stream, sIdx) => (
                ...
                <div className="flex items-center justify-between p-6 cursor-pointer group" onClick={() => toggleStream(stream._id)}>
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">{displayStreamName(stream.name)}</h2>
                            <div className="flex gap-3 mt-1">
                                <span className="text-[0.65rem] font-black uppercase tracking-widest bg-primary-100 dark:bg-primary-900/50 text-primary-600 px-2 py-0.5 rounded-md">{levelLabels[stream.level] || stream.level}</span>
