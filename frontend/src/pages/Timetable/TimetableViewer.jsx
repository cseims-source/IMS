...
import { Calculator, FlaskConical, BookText, Palette, Atom, TestTube, Sigma, Download, Trash2, AlertCircle, Sparkles, Loader2, ImageDown } from 'lucide-react';
...
function Timetable() {
    ...
    const exportToPdf = async () => {
        ...
        pdf.save(`timetable-${selectedStreamName}-${selectedSemester}.pdf`);
    };
    ...
    const exportToImage = async () => {
        ...
        link.download = `timetable-${selectedStreamName}-${selectedSemester}.png`;
    ...
    const stats = useMemo(() => {
        ...
        days.forEach(day => {
            timeSlots.forEach(time => {
                if (timetable[day]?.[time]) occupied += 1;
            });
        });
        return { totalSlots, occupied, free: totalSlots - occupied };
    }, [timetable]);
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="self-end flex gap-2">
                        ...
                        <button
                            ...
                            className="flex items-center px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition"
                        >
                            <ImageDown size={16} className="mr-2" /> Export PNG
                        </button>
                    </div>
                </div>
            </div>
            <div className="flex flex-wrap gap-4 mb-6">
                <div className="px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-xs font-semibold text-gray-700 dark:text-gray-200">
                    Total Slots: {stats.totalSlots}
                </div>
                <div className="px-4 py-2 rounded-lg bg-primary-100 dark:bg-primary-900/40 text-xs font-semibold text-primary-700 dark:text-primary-200">
                    Occupied: {stats.occupied}
                </div>
                <div className="px-4 py-2 rounded-lg bg-green-100 dark:bg-green-900/40 text-xs font-semibold text-green-700 dark:text-green-200">
                    Free: {stats.free}
                </div>
            </div>
            {suggestionError && (
