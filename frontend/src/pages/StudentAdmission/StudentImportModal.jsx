...
// Complete Institutional Logic Header Map for AIET
const HEADER_MAP = {
    "STUDENT NAME": "firstName", // Handled with splitting logic below
    "E-MAIL": "email",
                 ...
                 <div className="flex justify-between items-center mb-12 relative z-10">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-[0.6rem] font-bold text-gray-400 uppercase tracking-[0.5em] mt-3">Legacy Ledger Translation Sequence</p>
                        </div>
                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="p-8 bg-indigo-50 dark:bg-indigo-900/10 rounded-[3rem] border border-indigo-100 dark:border-indigo-800/50">
                                    ...
                                    <ul className="text-[0.65rem] text-indigo-600 dark:text-indigo-400 space-y-3 font-bold uppercase tracking-wider">
                                        <li className="flex items-center gap-3"><ArrowRight size={12}/> Every column in your Excel is mapped to the registry.</li>
                                        <li className="flex items-center gap-3"><ArrowRight size={12}/> Parent occupations & addresses: Syncing...</li>
                                        <li className="flex items-center gap-3"><ArrowRight size={12}/> 4-Year Fee Grids: Detected automatically.</li>
                                    </ul>
                                </div>
                                <label className="flex flex-col items-center justify-center w-full h-56 border-4 border-dashed border-gray-100 dark:border-gray-800 rounded-[3rem] cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all group overflow-hidden bg-gray-50/50 dark:bg-gray-950/30">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-[0.65rem] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">{file ? file.name : 'Inject Institutional Excel'}</p>
                                    </div>
                                ...
                                <div className="overflow-x-auto rounded-[2.5rem] border dark:border-gray-800 max-h-72 shadow-inner bg-gray-50 dark:bg-gray-950">
                                    <table className="w-full text-[0.6rem] text-left">
                                        <thead className="bg-gray-100 dark:bg-gray-900 sticky top-0 font-black uppercase tracking-widest text-gray-500">
                                            <tr>
                                                <th className="p-5 pl-8">Logic Index</th>
                                                <th className="p-5">Registry Identity</th>
                                                <th className="p-5">Course Node</th>
                                                <th className="p-5">Financial Matrix (Y1)</th>
                                                <th className="p-5 pr-8">Spatial Center</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y dark:divide-gray-800">
                                            {mappedData && mappedData.slice(0, 10).map((row, i) => (
                                                <tr key={i} className="text-gray-600 dark:text-gray-300 font-bold hover:bg-primary-500/5 transition-colors">
                                                    <td className="p-5 pl-8 font-mono text-indigo-500">{i + 1}</td>
                                                    <td className="p-5">{row.firstName} {row.lastName}</td>
                                                    ...
                                                    <td className="p-5 text-accent-600">₹{row.yearFees?.y1}</td>
                                                    <td className="p-5 pr-8 opacity-60 italic">{row.presentAddress?.city}, {row.presentAddress?.state}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                </div>
                                <p className="text-[0.55rem] text-gray-400 font-black uppercase text-center mt-2 italic">* Preview only shows key identifying nodes. Full data sequence is queued for upload.</p>
                            </div>
