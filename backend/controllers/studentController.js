...
const addStudent = async (req, res) => {
    try {
        ...
        const student = new Student({ firstName, lastName, email, phone, dob, gender, address, stream, currentSemester, photo });
...
const importStudents = async (req, res) => {
    ...
    const newStudents = [];
    const failedImports = [];
    const incomingEmails = students.map(s => s.email).filter(Boolean);
    ...
    try {
        ...
        students.forEach((student, index) => {
            if (!student.email || !student.firstName || !student.lastName) {
                ...
                failedImports.push({ student, reason: `Email '${student.email}' already exists.` });
            } else {
...
const getStudentFees = async (req, res) => {
    try {
        const studentId = req.user?.profileId || req.params.id;
